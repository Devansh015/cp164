...
class Movie:
    ...
    def genres_menu():
        ...
        -------------------------------------------------------
        """
        max_index_width = len(str(len(Movie.GENRES) - 1))
        menu_lines = ["Genres"]
        format_string = f"{{:>{max_index_width}}} {{}}"
        for index, genre in enumerate(Movie.GENRES):
            formatted_line = format_string.format(index, genre)
            menu_lines.append(formatted_line)
        return "\n".join(menu_lines)
...
Genres:   {}""".format(self.title, self.year, self.director, self.rating, genres_list)
    ...
    def genres_string(self):
        ...
        genre_names = [self.GENRES[index] for index in self.genres]
