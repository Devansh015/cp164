...
class BST:
    ...
    def parent_r(self, key):
        ...
        return self.parent_r_aux(self._root, key, None)

    def parent_r_aux(self, node, key, parent):
        ...
        # Searching left subtree
            ...
            value = self.parent_r_aux(node._left, key, node)

        # Searching right subtree
        elif key > node._value:
            value = self.parent_r_aux(node._right, key, node)

        # Val has been found without parent node
        elif parent is None:
            value = None

        # Value has been found returns parent val
        else:
            value = deepcopy(parent._value)
