...
def queue_test(a):
  ...
  non-empty queues using the data in a:
    ...
    print("Is the queue empty?", q.is_empty())  # Expected: True
    ...
    print("Is the queue empty?", q.is_empty())  # Expected: False

    # Test peek method
    if not q.is_empty():
        front = q.peek()
        print(f"Peeked at the front of the queue: {front}")  # Expected: First element of `a`

    # Test remove method
    while not q.is_empty():
        front = q.remove()
        print(f"Removed {front}: Queue length is now {len(q)}")

    # Final check if the queue is empty
    print("Is the queue empty after removals?", q.is_empty())  # Expected: True
...
def array_to_pq(pq, source):
    ...
    """
    while source:
        pq.insert(source.pop(0))  # Insert element at front of pq
...
def priority_queue_test(a):
    ...
    if not pq.is_empty():
        print("is_empty() failed on an empty priority queue.")
    ...
    if pq.is_empty():
        print("is_empty() failed on a non-empty priority queue.")

    # Test peek
    if pq.peek() != min(a):
        print("peek() failed, expected:", min(a), "but got:", pq.peek())

    # Test remove
    while not pq.is_empty():
        if pq.remove() != min(a):
            print("remove() failed, expected:", min(a), "but got:", pq.remove())
        a.remove(min(a))

    # Test is_empty after removing all elements
    if not pq.is_empty():
        print("is_empty() failed after removing all elements.")
        return
